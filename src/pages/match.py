...
with st.sidebar:
    with st.form("run", border=False):
        ...
        stopwords_collections = st.multiselect("Stopwords Collections", Db.get_collections("vocab"))
        ...
        if st.form_submit_button("Run"):
            ...
            st.session_state["stopwords_collections"] = stopwords_collections
            st.session_state["stopwords_vocab"] = Db.get_ids("vocab", stopwords_collections)
            st.session_state["experience_doc"] = Scrapper.find_keywords({"description": experience_text, "intitule": ""}, st.session_state["stopwords_vocab"])
            docs = []
            for document in Db.get_documents("jobs", jobs_collections):
                document["score"] = Scrapper.score_matching(document, st.session_state["experience_doc"],
                                                            st.session_state["stopwords_vocab"])
